...
public class UtilsGame {
    ...
    public int checkWon(int[][] field) {
        for(int i = 5; i >= 0; i++) {
            for(int y = 0; y < 7; y++) {
                if(field[i][y] != 0) {
                    int count1 = checkPoint(i, y, field[i][y], field, "up");
                    int count2 = checkPoint(i, y, field[i][y], field, "diagonal");
                    int count3 = checkPoint(i, y, field[i][y], field, "right");
    ...
    public int checkPoint(int x, int y, int value, int[][] field, String str) {
        int count = 1;
        if(x-1 >= 0 && str.equals("up")) {
            if(value == field[x-1][y]) {
                checkPoint(x - 1, y, field[x - 1][y], field, "up");
                count++;
            }
        } else if(x-1 >= 0 && y+1 < 6 && str.equals("diagonal")) {
            if(value == field[x - 1][y + 1]) {
                checkPoint(x - 1, y + 1, field[x - 1][y + 1], field, "diagonal");
                count++;
            }
        } else if(y+1 < 6 && str.equals("right")) {
            if(value == field[x][y + 1]) {
                checkPoint(x, y + 1, field[x][y + 1], field, "right");
                count++;
            }
        }
        return count;
